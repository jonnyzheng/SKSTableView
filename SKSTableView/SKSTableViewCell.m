...
{
    if (!_image) {
        _image = [UIImage imageNamed:@"down-arrows-normal.png"];
    }
...
//    CGRect frame = CGRectMake((point.x - CGRectGetWidth(bounds) * 1.5 ), point.y * 1.4 , CGRectGetWidth(bounds) * 3.0 , CGRectGetHeight(self.bounds) - point.y * 1.4 );
    ...
    CGRect frame = CGRectMake((point.x - CGRectGetWidth(bounds) * 0.5 ), point.y * 1.8 , CGRectGetWidth(bounds) * 1.0 , CGRectGetHeight(self.bounds) - point.y * 1.8 );
